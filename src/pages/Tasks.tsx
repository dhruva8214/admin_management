  ...
  const getFilteredTasks = (filter: string) => {
    switch (filter) {
      ...
      case "completed":
        return tasks.filter(task => task.completed);
      case "housekeeping":
        return tasks.filter(task => task.department === "housekeeping");
      case "maintenance":
        return tasks.filter(task => task.department === "maintenance");
      case "frontdesk":
        return tasks.filter(task => task.department === "frontdesk");
      default:
        return tasks;
    }
  };

  const renderTaskList = (filteredTasks: Task[], title: string) => (
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-lg font-medium">{title} ({filteredTasks.length})</CardTitle>
      </CardHeader>
      ...
      <div className="space-y-6">
        ...
        <Tabs defaultValue="all" className="w-full">
          ...
          <TabsContent value="all">
            {renderTaskList(getFilteredTasks("all"), "All Tasks")}
          </TabsContent>
          
          <TabsContent value="pending">
            {renderTaskList(getFilteredTasks("pending"), "Pending Tasks")}
          </TabsContent>
          
          <TabsContent value="completed">
            {renderTaskList(getFilteredTasks("completed"), "Completed Tasks")}
          </TabsContent>
          
          <TabsContent value="housekeeping">
            {renderTaskList(getFilteredTasks("housekeeping"), "Housekeeping Tasks")}
          </TabsContent>
          
          <TabsContent value="maintenance">
            {renderTaskList(getFilteredTasks("maintenance"), "Maintenance Tasks")}
          </TabsContent>
          
          <TabsContent value="frontdesk">
            {renderTaskList(getFilteredTasks("frontdesk"), "Front Desk Tasks")}
          </TabsContent>
        </Tabs>
      </div>
    </MainLayout>
